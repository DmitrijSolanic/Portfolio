...
while True:
    ...
    # Gathering inputs
    ticker = input("Please input ticker of your preferred asset (as shown on Yahoo finance, e.g. TSLA for Tesla/ AAPL for Apple/ ^GSPC for S&P500/ GC=F for Gold etc.): ")
    frequency = input("Please input frequency of contributions (w for weekly/ m for monthly/ y for yearly): ")
    ...
    # Re-creating DataFrame with predated date to be able to use lag later effectively.
    ...
    if frequency == 'w':
        ...
        # Checking whether day is the first day of a new week
        df['same_week'] = (df['week'] == df['lagged_week_filled']).astype(int)
        # Filtering df for the first day of every week
    ...
    # Adding last row with the latest price and appending the new row to the DataFrame
    ...
    if p_l < 0:
        print("Congratulations on not following that idea through. \n\nClose graph widnow to continue.")
    else:
        print("Don't be sad. Remember the saying 'The best time to start investing was 10 years ago, the next best time is now.' \n\nClose graph widnow to continue.")
